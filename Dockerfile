...
ARG AUUID="7b3cabcb-2ec9-4a0b-b5ea-db280fffff42"
...
RUN apk update && \
    ...
    cat /tmp/Caddyfile | sed -e "1c 0.0.0.0:$PORT" -e "s/\$AUUID/$AUUID/g" -e "s/\$MYUUID-HASH/$(caddy hash-password --plaintext $AUUID)/g" >/etc/caddy/Caddyfile && \
